...
namespace Core.Services
{
    public class AudioSourceService : MonoConstruct, ISerializableService
    {
        ...
        private readonly Glossary<AudioSource> _sources = new();

        [SerializeField] private AudioMixer _audioMixer;
        public float MasterVolume => _serviceData.masterVolume;
        public float MusicVolume => _serviceData.musicVolume;
        ...
        public void ReloadData()
        {
            ...
            SetMasterVolume(_serviceData.masterVolume);
            SetMusicVolume(_serviceData.musicVolume);
            SetSFXVolume(_serviceData.sfxVolume);
            UpdateMasterVolume();
            OnReload?.Invoke();
        }
        ...
        public void SetMasterVolume(float percent)
        {
            ...
            float volumeDb = percent > 0 ? Mathf.Log10(percent) * 20f : -80f;
            _audioMixer.SetFloat(MasterKey, volumeDb);
            UpdateMasterVolume();
        ...
        {
            percent = Math.Clamp(percent, 0, 1);
            _serviceData.musicVolume = percent;
            float volumeDb = percent > 0 ? Mathf.Log10(percent) * 20f : -80f;
            _audioMixer.SetFloat(MusicKey, volumeDb);
        ...
        public void SetSFXVolume(float percent)
        {
            ...
            // -80 dB считается полной тишиной в Unity
            float volumeDb = percent > 0 ? Mathf.Log10(percent) * 20f : -80f;
            _audioMixer.SetFloat(SFXKey, volumeDb);
            _playerDataService.SetDirty(this);
        }
        ...
        public void PlayOneShot(AudioSource sourcePrefab, Vector3 position)
        {
            ...
            source.volume = sourcePrefab.volume;
            source.transform.position = position;
            source.PlayOneShot(sourcePrefab.clip);
        }

        public void StopOneShot(AudioSource sourcePrefab)
        {
            if (!Application.isPlaying)
                return;

            if (!_sources.TryGetValue(sourcePrefab.GetInstanceID(), out _))
                ExtractSources(sourcePrefab.gameObject);

            _sources.GetValue(sourcePrefab.GetInstanceID()).Stop();
        }
        ...
        )
        {
            ...
            while (true)
            {
                ...
                {
                    backgroundOld.volume -= backgroundOldBaseVolume * Time.unscaledDeltaTime / fadeTime;
                ...
                var totalMaxCurrentVolume = backgroundCurrentBaseVolume;
        ...
        {
            if (_backgroundCurrent is not null)
                _backgroundCurrent.volume = _backgroundCurrentBaseVolume * BackgroundTempScale;
        }
