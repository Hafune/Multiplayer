...
namespace Core
{
     ...
     */
    public static class FormatFloatToStringUtility
    {
        public static string ToInt(float value) => ((int)value).ToString();
        public static string ToPercentInt(float value) => $"{(int)Math.Round(value * 100)}";
        public static string WeaponDamagePerSecond(float value) => $"{value,12:# ###.0}".Trim();
        public static string FloatDim1(float value) => $"{value,18:0.0}".Trim();
        public static string FloatDim2(float value) => $"{value,18:0.00}".Trim();
    }
}