...
namespace Lib
{
    ...
    {
        public static void InitializeWIth<T>(this T[] array, Func<T> initFunc)
        {
            for (int t0 = 0; t0 < array.GetLength(0); t0++)
                array[t0] = initFunc.Invoke();
        }
        ...
        public static T MinBy<T>(this T[] list, Func<T, float> compare)
        {
        ...
        public static Vector2 SumBy<T>(this IEnumerable<T> list, Func<T, Vector2> callback)
        {
        ...
        public static float SumBy<T>(this IList<T> list, Func<T, float> callback)
        {
            var agg = 0f;
        ...
        public static T Random<T>(this IList<T> list, int length) => list[UnityEngine.Random.Range(0, length)];
        public static T Random<T>(this IList<T> list) => list[UnityEngine.Random.Range(0, list.Count)];
        public static T Random<T>(this IList<T> list, MyRandom random) => list[random.NextInt(0, list.Count)];
        public static int RandomIndex<T>(this IList<T> list) => UnityEngine.Random.Range(0, list.Count);
        public static int RandomIndex<T>(this IList<T> list, MyRandom random) => random.NextInt(0, list.Count);
        ...
        public static T CircularNext<T>(this T[] array, T item) =>
            Circular(array, Array.IndexOf(array, item) + 1);

        public static T CircularPrevious<T>(this T[] array, T item) =>
            Circular(array, Array.IndexOf(array, item) - 1);

        public static int CircularNextIndex<T>(this T[] array, T item) =>
            CircularIndex(array, Array.IndexOf(array, item) + 1);

        public static int CircularPreviousIndex<T>(this T[] array, T item) =>
            CircularIndex(array, Array.IndexOf(array, item) - 1);

        public static T Circular<T>(this T[] array, int index) => 
            array[CircularIndex(array, index)];

        public static int CircularIndex<T>(this T[] array, int index)
        {
            var _index = index % array.Length;

            if (_index < 0)
                _index = array.Length + _index;

            return _index;
        }

        public static void SortBy<T, T2>(this List<T> list, Func<T, T2> keySelector) where T2 : System.IComparable =>
