...
namespace Lib
{
    public static class MyLeoEcsExtensions
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void DelIfExist(this IEcsPool pool, int entity)
        {
            if (!pool.Has(entity))
                return;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int GetFirst(this EcsFilter filter) => filter.GetEntitiesCount() == 0 ? -1 : filter.GetRawEntities()[0];
            ...
            where T6 : struct 
        {
            public EcsWorld.Mask Fill (EcsWorld.Mask mask) {
                return mask.Exc<T1> ().Exc<T2> ().Exc<T3> ().Exc<T4> ().Exc<T5> ().Exc<T6> ();
            }
        }
        ...
        {
            public EcsWorld.Mask Fill (EcsWorld.Mask mask) {
                return mask.Exc<T1> ().Exc<T2> ().Exc<T3> ().Exc<T4> ().Exc<T5> ().Exc<T6> ().Exc<T7> ();
            }
        }
    }
}