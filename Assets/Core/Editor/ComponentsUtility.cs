...
namespace Core.Editor
{
    public static class ComponentsUtility
    {
        ...
        private static string[] GetValueComponentNames() => ComponentNamesCache ??= MyFunctions.GetAssignableTypes<IValue>()
            .Select(i => i.Name)
            .OrderBy(i => i)
            .ToArray();

        public static string[] GetSlotTagNames() => SlotTagNamesCache ??= MyFunctions.GetAssignableTypes<ISlotTag>()
            .Select(i => i.Name)
            .OrderBy(i => i)
            .ToArray();

        private static string[] GetGenericValueComponentNames()
        {
            _genericNames ??= MyFunctions.GetAssignableTypes<IValue>(true)
                .Select(i => i.Name.Remove(i.Name.IndexOf('`')))
                .OrderBy(i => i)
                .ToArray();

            if (_genericNames.Length == 0)
            ...
            _guids ??= AssetDatabase.FindAssets($"t:Script",
                new[]
                {
                    ...
                    "Assets/Core/EcsCommon/Features",
                });
        ...
        private static string[] GetAllComponentNames()
        {
