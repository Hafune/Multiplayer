    ...
    {{
        ...
        internal static bool Gen()
        {
            ...
            var filters = totalNames.SelectMany(name => new[]
            {
                ...
                $"private readonly EcsFilterInject<Inc<{name}, EventStartRecalculateAllValues, BaseValueComponent<{name}>>> _base{R(name)}Filter;",
            });
            ...
            {
                $"if (_eventUpdated{R(name)}Filter.Value.GetEntitiesCount() != 0) foreach (var i in _eventUpdated{R(name)}Filter.Value) _eventUpdated{R(name)}Pool.Value.Del(i);",
                $"if (_base{R(name)}Filter.Value.GetEntitiesCount() != 0) foreach (var i in _base{R(name)}Filter.Value) {{ _{R(name)}Pool.Value.Get(i).value = _base{R(name)}Pool.Value.Get(i).baseValue; _eventUpdated{R(name)}Pool.Value.Add(i); }}",
            });
