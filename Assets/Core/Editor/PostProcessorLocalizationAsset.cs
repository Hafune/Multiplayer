...
{
    public class PostProcessorLocalizationAsset : AssetPostprocessor
    {
        private const string PATH = "/Core/i18n/";
        private const string TEMPLATE_PATH = "Assets/Core/Editor/Resources/i18n.cs.txt";
        ...
        private static bool IsSmart(StringTableCollection tableCollection, long id)
        {
            ...
            return tableCollection.StringTables.Select(stable =>
                    stable.GetEntry(id)).Any(tableEntry => tableEntry?.IsSmart == true);
        }
        ...
        private static string GenerateFileContent(SharedTableData table)
        {
            ...
            foreach (var entry in table.Entries)
            {
                ...
                sb.Append($"\t\t\tpublic static long {key}_key = {entry.Id};");
                sb.Append(Environment.NewLine);
            }
            return string.Format(
                template, KeyToCSharp(table.TableCollectionName), table.TableCollectionName, sb);
        }
