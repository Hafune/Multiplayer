...
{
    private readonly string dir = "Assets/Prefabs/Level/Arts";
    ...
    protected override IEnumerator ExecuteProtected(Transform[] suitable, Action callback, Action cancel)
    {
        ...
        foreach (var i in suitable)
        {
            ...
            {
                if (!a.TryGetComponent<PrefabCompressedData>(out var data))
                    data = a.gameObject.AddComponent<PrefabCompressedData>();
                throw new Exception();
