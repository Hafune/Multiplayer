...
namespace DamageNumbersPro
{
    public abstract class DamageNumber : MonoBehaviour
    {
        ...
        public string spamGroup = string.Empty;
        ...
        readonly List<(MeshRenderer, MeshRenderer)> subMeshRenderers = new();
        readonly List<(MeshFilter, MeshFilter)> subMeshFilters = new();
        protected readonly List<Mesh> meshs = new();
        protected readonly List<List<Color>> colors = new();
        protected readonly List<List<float>> alphas = new();
        private readonly List<List<Color>> _colorsCache = new();
        private readonly List<List<float>> _alphasCache = new();
        private readonly List<Color> _colorsEmpty = new();
        private readonly List<float> _alphasEmpty = new();
        ...
        //Spam Control:
        static readonly Dictionary<string, HashSet<DamageNumber>> spamGroupDictionary = new();
        ...
        static readonly Dictionary<int, HashSet<DamageNumber>> pools = new ();
        private static readonly HashSet<DamageNumber> activeInstances = new();
        int poolingID;
        bool performRestart;
        bool destroyAfterSpawning;

        //Fallback font fix.
        static readonly Dictionary<TMP_FontAsset, GameObject> fallbackDictionary = new ();
        ...
        */
        ...
        public virtual void GetReferences()
        {
            ...
            for (int n = 0; n < parentA.childCount; n++)
            {
                ...
                subMeshRenderers.Add((childA.GetComponent<MeshRenderer>(), childB.GetComponent<MeshRenderer>()));
                subMeshFilters.Add((childA.GetComponent<MeshFilter>(), childB.GetComponent<MeshFilter>()));
            }
        }
        ...
        public void UpdateText()
        {
            ...
            if (enableNumber)
            {
                ...
                {
                    numberString = ProcessIntegers(IntStringCache.Get(Mathf.RoundToInt(number)), out shortened);
                }
                else
                {
                    //Digits:
                    string allDigits = IntStringCache.Get(Mathf.RoundToInt(Mathf.Abs(number) * Mathf.Pow(10, digitSettings.decimals)));
            ...
            for (int n = 0; n < meshs.Count; n++)
            {
                ...
                if(mesh != null)
                {
                    if (cacheIndex >= _colorsCache.Count)
                    {
                        _colorsCache.Add(new());
                        _alphasCache.Add(new());
                    }

                    var color = _colorsCache[cacheIndex];
                    mesh.GetColors(_colorsCache[cacheIndex]);
                    var alpha = _alphasCache[cacheIndex];
                    alpha.Clear();

                    while (alpha.Count < color.Count) 
                        alpha.Add(color[alpha.Count].a);
                ...
                {
                    colors.Add(_colorsEmpty);
                    alphas.Add(_alphasEmpty);
                }
            }
            ...
            for (int c = 0; c < textMeshPro.transform.childCount; c++)
            {
                ...
                if (subMeshRenderer != null)
                {
                    ...
                    if (subMeshRenderers.Count <= c)
                    {
                        ...
                        subMeshRenderers.Add((subMeshA.GetComponent<MeshRenderer>(), subMeshB.GetComponent<MeshRenderer>()));
                        subMeshFilters.Add((subMeshA.GetComponent<MeshFilter>(), subMeshB.GetComponent<MeshFilter>()));
                    }
            ...
            if(meshs != null)
            {
                for (int n = 0; n < meshs.Count; n++)
                {
                    if (colors[n] != null && meshs[n] != null)
                    {
                        ...
                        {
                            var col = color[c]; 
                            col.a = alphaFactor * alpha[c];
                            color[c] = col;
                        }

                        meshs[n].SetColors(color);
                    }
                }
            }
        ...
        {
            if (!removedFromDictionary && spamGroup != string.Empty)
            {
